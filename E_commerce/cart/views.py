...
def add_cart(request, product_id):
    ...
    try:
        cart = Cart.objects.get(cart_id=_cart_id(request)) # get the cart in the cart_id present in the session
    except:
        cart = Cart.objects.create(cart_id=_cart_id(request))
    cart.save()

    is_create_item_exists = CartItem.objects.filter(product=product, cart=cart).exists()
    if is_create_item_exists:
...
def remove_cart_item(request, cart_item_id):
    try:
        ...
        messages.info(request, "This item was already removed from your cart.")
    return redirect('cart')
